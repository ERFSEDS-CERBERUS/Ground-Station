...
def extract(mode="live"):
    ...
    match mode:
        case "live":
            ...
            print(
                "Input the exact name of the file you wish to process\nExample: foo_Bar.json\n"
            )
            ...
            f = open(
                "/home/pciii/nova-ground-station-sw/post_flight_analysis/raw_data/"
                + f_name,
                "r",
            )
            ...
            print("Processing " + str(len(data)) + " messages")
            ...
            print(
                "Generating zeros arrays for each data field at "
                + str(resolution)
                + "subintervals per second\nTotal data points per field: "
                + str(resolution * flight_time)
            )
            print("Generating time zeros array...")
            time = np.zeros(
                int(
                    (np.ceil(resolution * flight_time)),
                )
            )
            ...
            high_G_x_raw = np.zeros(
                (high_G_points),
            )
            high_G_y_raw = np.zeros(
                (high_G_points),
            )
            high_G_z_raw = np.zeros(
                (high_G_points),
            )
            high_G_time_raw = np.zeros(
                (high_G_points),
            )
            high_G_points = 0
            for i, v in enumerate(data):
            ...
            for _, v in enumerate(data):
                if list(v["data"].keys()) == ["BarometerCalibration"]:
                    pressure_sensitivity = v["data"]["BarometerCalibration"][
                        "pressure_sensitivity"
                    ]
                    pressure_offset = v["data"]["BarometerCalibration"][
                        "pressure_offset"
                    ]
                    temperature_coefficient_ps = v["data"]["BarometerCalibration"][
                        "temperature_coefficient_ps"
                    ]
                    temperature_coefficient_po = v["data"]["BarometerCalibration"][
                        "temperature_coefficient_po"
                    ]
                    reference_temperature = v["data"]["BarometerCalibration"][
                        "reference_temperature"
                    ]
                    temperature_coefficient_t = v["data"]["BarometerCalibration"][
                        "temperature_coefficient_t"
                    ]
            ...
            pressure_raw = np.zeros(
                (pressure_points),
            )
            pressure_time_raw = np.zeros(
                (pressure_points),
            )
            pressure_points = 0
            for i, v in enumerate(data):
                if list(v["data"].keys()) == ["BarometerData"]:
                    pressure_raw[pressure_points] = v["data"]["BarometerData"][
                        "pressure"
                    ]
            ...
            print(
                "Note: If you are seeing this message it means that accounting for calibration has not been implemented"
            )
            ...
            temperature_raw = np.zeros(
                (temperature_points),
            )
            temperature_time_raw = np.zeros(
                (temperature_points),
            )
            temperature_points = 0
            for i, v in enumerate(data):
                if list(v["data"].keys()) == ["BarometerData"]:
                    temperature_raw[temperature_points] = v["data"]["BarometerData"][
                        "temprature"
                    ]  # *temprature*
                    temperature_time_raw[temperature_points] = time_raw[i]
                    v_points = temperature_points + 1
            print("Using linear interpolation to match pressure to time array...")
            temperature = np.interp(time, temperature_time_raw, temperature_raw)
            ...
            gyro_x = np.interp(time, gyro_time_raw, gyro_x_raw)
            gyro_y = np.interp(time, gyro_time_raw, gyro_y_raw)
            gyro_z = np.interp(time, gyro_time_raw, gyro_z_raw)
        ...
        case "from_file":
            ...
            TestData = pd.read_csv(
                os.path.join(
                    os.path.dirname(os.path.dirname(__file__)), "raw_data", "yae.csv"
                ),
                usecols=col_list,
            )
            # Gather all the info from the columns
            time, pressure, x_accel, y_accel, z_accel, x_ang, y_ang, z_ang = (
                TestData["time"],
                TestData["pressure"],
                TestData["x_acceleration"],
                TestData["y_acceleration"],
                TestData["z_acceleration"],
                TestData["x_angle"],
                TestData["y_angle"],
                TestData["z_angle"],
            )
